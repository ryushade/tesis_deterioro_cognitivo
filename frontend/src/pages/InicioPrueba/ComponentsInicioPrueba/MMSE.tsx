...
export default function MMSE() {
  ...
  const [mmseByPaciente, setMmseByPaciente] = useState<EvalMap>({});
    ...
    <div className="space-y-4">
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              pacientes.map((p) => {
                const evalItem = mmseByPaciente[p.id_paciente] || null;
                return (
                  <TableRow key={p.id_paciente}>
                    ...
                    <TableCell className="whitespace-nowrap">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${sexoBadgeClass(p.sexo)}`}>
                        {formatSexo(p.sexo, p.sexo_display)}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="whitespace-nowrap">
                      {evalItem ? (
                        ...
                        <Badge variant="outline">Sin MMSE</Badge>
                      )}
                    </TableCell>
                    <TableCell className="whitespace-nowrap">
      ...
      <Sheet open={detalleOpen} onOpenChange={setDetalleOpen}>
        <SheetContent side="right">
          <SheetHeader>
            <SheetTitle>Detalle MMSE {detallePaciente ? `— ${detallePaciente.nombre_completo}` : ''}</SheetTitle>
          </SheetHeader>
          <div className="p-4 space-y-4">
            {!detallePaciente ? (
              ...
              <div className="text-sm text-gray-500">El paciente no registra evaluaciones MMSE.</div>
            ) : (
